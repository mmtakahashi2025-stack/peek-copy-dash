...
export function SheetDataProvider({ children }: { children: ReactNode }) {
  ...
  const getKpis = useCallback((filialId: string, dateFilter?: DateFilter, leadsRecebidos?: number): KpiData[] => {
    ...
    // Faturamento and Lucro both EXCLUDE 'PC' (Pacote) type items
    const nonPcData = filteredData.filter(r => r.Tipo !== 'PC');
    
    // Vendas: unique venda IDs (excluding PC items for counting)
    const vendaIds = new Set(nonPcData.map(r => r['Venda #']));
    const totalVendas = vendaIds.size;
    const totalFaturamento = nonPcData.reduce((sum, r) => sum + (r.Líquido || 0), 0);
    // CORRECTED: Lucro also EXCLUDES PC types (validated against ERP)
    const totalLucro = nonPcData.reduce((sum, r) => sum + (r.Lucro || 0), 0);
    const totalQuantidade = nonPcData.reduce((sum, r) => sum + (r.Quantidade || 0), 0);
